...
const PokemonDetails: React.FC<PokemonDetailsProps> = ({ pokemon }) => {
  ...
  return (
    <div className="flex flex-col md:flex-row items-center md:items-start justify-center gap-6 md:gap-10 bg-gray-950 rounded-lg p-4 md:p-8 shadow-md border border-gray-200 dark:border-gray-700 w-full max-w-4xl mx-auto">
      <div className="flex flex-col justify-center items-center gap-3 flex-shrink-0 w-full md:w-auto">
        <div className={`relative w-48 h-48 sm:w-56 sm:h-56 md:w-64 md:h-64 bg-gradient-to-br from-gray-950 via-amber-950 to-yellow-500 rounded-lg p-2 border-2 shadow-inner transition-all duration-300 ease-in-out ${pokemon.is_legendary ? 'border-yellow-400 shadow-lg shadow-yellow-400/50 hover:border-yellow-300' : 'border-gray-300 dark:border-gray-600 hover:border-yellow-500 hover:shadow-lg'}`}>
          <Image 
            src={imageUrl} 
            alt={pokemon.name} 
            fill 
